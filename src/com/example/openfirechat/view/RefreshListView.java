...
public class RefreshListView extends ListView implements OnScrollListener{
    ...
    private boolean mUseAutoMore = false;
    
    private static final int AUTO_LOADING = 10;
    
    public void setOnRefreshListener(RefreshListener refreshListener, boolean useAutoMore) {
        this.mRefreshListener = refreshListener; 
        mUseAutoMore = useAutoMore;
    }
    ...
    void init(final Context context) {
        ...
        //addHeaderView(mHeaderLinearLayout, null, true);
        addHeaderView(mHeaderLinearLayout);

        mRefreshLayout = mHeaderLinearLayout.findViewById(R.id.refresh_layout);
        //RefreshListView.this.findViewById(R.id.refresh_layout).setVisibility(View.GONE);
...
//    	Log.v(TAG, "onScroll, first: " + firstVisibleItem + 
//    			", vcount: " + visibleItemCount + ", count: " + totalItemCount);    	
...
//    			", visibleItemCount: " + visibleItemCount);
    	...
    	if (firstVisibleItem == 0 && 
    		...
    		mRefreshLayout.getVisibility() == View.GONE) {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        try {
	        int lastIndex = this.getAdapter().getCount();
	        if (mUseAutoMore && mRefreshListener != null && mFooterTextView != null &&
	        	scrollState == OnScrollListener.SCROLL_STATE_IDLE && 
	        	getFooterViewsCount() > 0 && 
	        	mLastVisiableItem >= lastIndex) {
	        	Object obj = mFooterTextView.getTag();
	        	if (obj == null || !(obj instanceof Integer) || (Integer)obj != AUTO_LOADING) {
	        		mFooterTextView.setTag(AUTO_LOADING);
	                mFooterTextView.setText(R.string.list_footer_loading);
	                mFooterProgressBar.setVisibility(View.VISIBLE);
	                mRefreshListener.autoMore();
	            }
	        }
        } catch (Exception e) {
        	e.printStackTrace();
    ...
    public void updateFootColor(int color) {
    	if (mFooterTextView != null)
    		mFooterTextView.setTextColor(color);
    }
    
    public void updateHeaderColor(int color) {
    	if (mHeaderTextView != null)
    		mHeaderTextView.setTextColor(color);
    	
    	if (mHeaderUpdateText != null)
    		mHeaderUpdateText.setTextColor(color);
    }
    
    public void updateProgressBar(final Drawable d) {
//    	if (mHeaderProgressBar != null)
//    		mHeaderProgressBar.setIndeterminateDrawable(d);
//    	
//    	if (mFooterProgressBar != null)
//    		mFooterProgressBar.setIndeterminateDrawable(d);
    }
